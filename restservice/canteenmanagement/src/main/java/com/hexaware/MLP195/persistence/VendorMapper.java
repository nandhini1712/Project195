...
public class VendorMapper implements ResultSetMapper<Vendor> {
  ...
  public final Vendor map(final int idx, final ResultSet rs, final StatementContext ctx) throws SQLException {
       ...
       * @return Vendor
       */
    return new Vendor(rs.getInt("VEN_ID"), rs.getString("VEN_NAME"), rs.getInt("VEN_PHNO"), rs.getString("ORD_STATUS") , rs.getString("DEL_STATUS") , rs.getFloat("ORD_COST") , rs.getInt("ORD_TOKEN_NUMBER"));
  }
}
